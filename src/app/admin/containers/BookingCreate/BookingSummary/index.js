...
const BookingSummary = (props) => {
  ...
  const check_in = bookingState?.dates
    ...
    : new Date();
  const check_out = bookingState?.dates
    ? dayjs(bookingState?.dates[1], 'YYYY-MM-DD').toDate()
    : new Date();
  ...
  return (
    <Card title="Booking Summary" foo>
      <div className="flex flex-col">
        ...
        <span>{new Date(check_in).toDateString()} - 2:00PM</span>
        <span className="font-bold ">Check-out:</span>
        <span>
          {new Date(check_out).toDateString()} - 11:00AM ({handleGetNoNights()}{' '}
          Nights)
        </span>
        <Divider />
