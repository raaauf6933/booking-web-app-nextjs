...
const createDataRows = (data) => {
  const rowBody = data.map((booking, index, array) => {
    ...
    booking.billing.charges_total = 0
...
export const dd = (data, formData, user) => {
  ...
  return {
    content: [
      ...
      "\n",
      {
        text: `FROM : ${moment(formData.dates[0].toDate()).format("LL")}`,
        style: "title1",
      },
      {
        text: `TO : ${moment(formData.dates[1].toDate()).format("LL")}`,
        style: "title1",
      },
      "\n",
      {
